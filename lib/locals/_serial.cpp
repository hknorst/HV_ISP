...
{ // Waits for a Serial char (blocking!)
...
void fillBuff(int n)
{ // Collects Serial chars and saves to the buffer
  for (int x = 0; x < n; x++)
    buff[x] = getSerialChar();
}

void empty_reply()
{ // Sends an OK / NOT OK signal
...
{ // Sends an OK + byte / NOT OK signal
  if (CRC_EOP == getSerialChar())
  {
    SERIAL.print((char)STK_INSYNC);
    SERIAL.print((char)b);
...
{ // Sends version info over Serial
  switch (c)
  {
  case 0x80:
    breply(HWVER);
...
void universal()
{
  ...
  else if (buff[0] == 0xAC && buff[1] == 0x80 && buff[2] == 0 && buff[3] == 0)
  {                                  // Erase Chip
  ...
  else if (buff[0] == 0x58 && buff[1] == 0x08 && buff[2] == 0 && buff[3] == 0)
  { // Read High Fuse
    writeHV(0x04, 0x4C);
    writeHV(0x00, 0x7A);
    breply(writeHV(0x00, 0x7E));
  }
  else if (buff[0] == 0xAC && buff[1] == 0xA8 && buff[2] == 0)
  { // Write High Fuse - buff[3] contains value
    writeHV(0x40, 0x4C);
    writeHV(buff[3], 0x2C);
    writeHV(0x00, 0x74);
    writeHV(0x00, 0x7C);
  ...
  else if (buff[0] == 0x50 && buff[1] == 0 && buff[2] == 0 && buff[3] == 0)
  { // Read Low Fuse
    writeHV(0x04, 0x4C);
    writeHV(0x00, 0x68);
    breply(writeHV(0x00, 0x6C));
  }
  else if (buff[0] == 0xAC && buff[1] == 0xA0 && buff[2] == 0)
  { // Write Low Fuse - buff[3] contains value
    writeHV(0x40, 0x4C);
    writeHV(buff[3], 0x2C);
    writeHV(0x00, 0x64);
    writeHV(0x00, 0x6C);
  ...
  else if (buff[0] == 0x50 && buff[1] == 0x08 && buff[2] == 0 && buff[3] == 0)
  { // Read Ext Fuse
    writeHV(0x04, 0x4C);
    writeHV(0x00, 0x6A);
    breply(writeHV(0x00, 0x7C));
  }
  else if (buff[0] == 0xAC && buff[1] == 0xA4 && buff[2] == 0)
  { // Write Ext Fuse - buff[3] contains value
    writeHV(0x40, 0x4C);
    writeHV(buff[3], 0x2C);
    writeHV(0x00, 0x66);
    writeHV(0x00, 0x6C);
